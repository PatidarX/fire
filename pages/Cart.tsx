...
const Cart: React.FC = () => {
  ...
  if (cart.length === 0) {
    return (
      <div className="pt-40 pb-40 flex flex-col items-center justify-center text-center px-6 min-h-screen bg-slate-950">
        ...
        <Link to="/hosting" className="px-10 py-4 fire-gradient rounded-2xl text-white font-black shadow-xl neon-orange transition-all hover:scale-105 uppercase tracking-widest text-sm">
          View Hosting Plans
        </Link>
      </div>
    );
  ...
  return (
    <div className="pt-32 pb-32 min-h-screen bg-slate-950">
      <div className="container mx-auto px-6">
        <h1 className="text-4xl md:text-5xl font-black text-white mb-12 tracking-tighter uppercase">Shopping <span className="text-orange-500">Cart</span></h1>
        
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          <div className="lg:col-span-2 space-y-6">
            {cart.map((item) => {
              const basePrice = item.cycle === BillingCycle.MONTHLY ? item.plan.priceMonthly : item.plan.priceYearly;
              ...
              return (
                <div key={item.id} className="glass p-8 rounded-[2rem] border-slate-800 flex flex-col md:flex-row justify-between gap-8 group">
                  <div className="flex-grow">
                    ...
                    <h3 className="text-2xl font-black text-white mb-2 uppercase">{item.plan.name} Hosting</h3>
          ...
          <div className="lg:sticky lg:top-32 space-y-6">
            <div className="glass p-8 rounded-[2.5rem] border-slate-800 shadow-2xl">
                ...
                <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
                  <div className="relative flex-grow mr-4">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 pl-12 pr-4 text-sm font-bold text-white focus:border-orange-500 outline-none"
                    />
                  </div>
                  <button className="bg-slate-800 hover:bg-slate-700 text-white px-6 py-3 rounded-xl font-bold text-sm transition-all uppercase tracking-widest">Apply</button>
                </div>
              </div>

              <div className="pt-6 border-t border-slate-800 mb-8">
                <div className="flex justify-between items-center">
                  <span className="text-lg font-black text-white uppercase">Grand Total</span>
              ...
              <button 
                ...
                className="w-full py-5 fire-gradient rounded-2xl text-xl font-black text-white shadow-xl neon-orange active:scale-95 transition-all mb-4 flex items-center justify-center space-x-3 uppercase tracking-widest"
              >
